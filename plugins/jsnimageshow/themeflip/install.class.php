...
class plgjsnimageshowThemeFlipInstallerScript
{
	...
	private function _updateSchema()
	{
		...
		$eid = $row->find(array('element' => 'themeflip', 'type' => 'plugin'));
		if ($eid)
		{
			$db = JFactory::getDBO();
	...
	private function _getInfo($id)
	{
		...
		$query->where('element=\'themeflip\' AND type=\'plugin\' AND folder=\'jsnimageshow\' AND extension_id = ' . $id);
		$db->setQuery($query);
