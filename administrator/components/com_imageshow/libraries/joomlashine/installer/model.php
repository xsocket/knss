...
class JSNInstallerModel extends JSNBaseModel
{
	...
	public function download()
	{
		...
		$input 		= $app->input;
		$extension 	= (object) $input->getArray($_GET);
	...
	public function install()
	{
		...
		// Get download link
		$url = $this->getLink($extension, $JVersion);
		$file = $input->files->get('package', null, 'raw');
		...
		if (is_array($file) && count($file))
		{
			...
			if ($file['error'] || $file['size'] < 1 || $file['name'] !== JFile::makeSafe($file['name'])	)
			{
				throw new Exception('FAIL:' . JText::_('JSN_EXTFW_INSTALLER_PACKAGE_SAVING_FAILED'));
			}
			...
			if (!JFile::upload($file['tmp_name'], $this->config->get('tmp_path') . '/' . $file['name'], false, true))
			{
				throw new Exception('FAIL:' . JText::_('JSN_EXTFW_INSTALLER_PACKAGE_SAVING_FAILED'));
			}

			$this->input->set('package', $file['name']);
		}
